...
rl.question('\nPlease enter your Supabase Database Password: ', async (password) => {
  ...
  try {
    console.log('\nExecuting homepage_images table migration...');
    const filePath = path.join(__dirname, '../supabase/migrations/20260607000000_homepage_images.sql');
    const sql = fs.readFileSync(filePath, 'utf8');

    await client.query(sql);
    console.log('Migration executed successfully!');
    console.log('The "homepage_images" table, RLS policy, triggers, and RPC fallbacks have been registered.');
  } catch (err) {
