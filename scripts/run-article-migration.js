...
console.log('----------------------------------------------------');
console.log('Supabase Article Number Migration Deployer');
console.log('----------------------------------------------------');
...
rl.question('Please enter your Supabase Database Password: ', async (password) => {
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, '../supabase/migrations/20260617150000_add_article_number.sql');
    const sql = fs.readFileSync(migrationPath, 'utf8');

    console.log('Deploying Article Number SQL migration...');
    await client.query(sql);
    console.log('\nMigration executed successfully!');
    console.log('The products table has been altered to support article numbers (format: bu-p#001).');
  } catch (err) {
