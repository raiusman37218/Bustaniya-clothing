...
console.log('----------------------------------------------------');
console.log('Supabase Stock Entries RPC Fallback Migration Deployer');
console.log('----------------------------------------------------');
...
rl.question('Please enter your Supabase Database Password: ', async (password) => {
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, '../supabase/migrations/20260618010000_add_stock_entries_rpc_fallback.sql');
    const sql = fs.readFileSync(migrationPath, 'utf8');

    console.log('Deploying Stock Entries RPC Fallback SQL migration...');
    await client.query(sql);
    console.log('\nMigration executed successfully!');
    console.log('The RPC functions for stock entries have been created and granted execute permissions.');
  } catch (err) {
